...
struct LSTMClassifier {
	...
	p_att2attexp = model.add_parameters({ATTENTION_HIDDEN_DIM});
        p_att_poolR = model.add_parameters({3, 2*HIDDEN_DIM});
	...
	p_attbias_l = model.add_parameters({ATTENTION_HIDDEN_DIM});
        ...
        p_att_poolR_l = model.add_parameters({3, 2*HIDDEN_DIM});
	...
	p_attbias_r = model.add_parameters({ATTENTION_HIDDEN_DIM});
        ...
        p_att_poolR_r = model.add_parameters({3, 2*HIDDEN_DIM});   
	...
	Expression i_att_poolR = parameter(cg, p_att_poolR);
	...
	Expression attbias_l = parameter(cg, p_attbias_l);
        ...
        Expression i_att_poolR_l = parameter(cg, p_att_poolR_l);
	...
	Expression attbias_r = parameter(cg, p_attbias_r);
        ...
        Expression i_att_poolR_r = parameter(cg, p_att_poolR_r);
...
if(DEBUG)   cerr<<"prob_value "<<prob_value<<" gold label "<<label<<"\n";
	...
	return output_loss;
*/
	Expression i_r_t = tanh(i_bias + i_att_poolR * att_pool + i_att_poolR_l * att_pool_l + i_att_poolR_r * att_pool_r);
